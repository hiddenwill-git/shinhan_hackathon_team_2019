...
%% req, module
multipart(Req, M) ->
    multipart(Req, M, ?MAX_FILE_SIZE_LIMIT).
multipart(Req, M, MaxFileSizeLimit) when is_atom(M) and is_integer(MaxFileSizeLimit) ->
    case cowboy_req:part(Req) of
        {ok, Headers, Req2} ->
            Req5 = case cow_multipart:form_data(Headers) of
                {data, FieldName} ->
                    {ok, Body, Req3} = cowboy_req:part_body(Req2),
                    ok = M:data_payload(FieldName, Body),
                    Req3;
                {file, FieldName, Filename, _CType, _CTransferEncoding} ->
                    ...
                    case Rsf of
                            {ok, FileSize, Req4} ->
                                ok = M:file_payload(FieldName, Filename, TempFilename, FileSize),
                                Req4;
                            {limit, Reason, Req4} ->
            ...
            multipart(Req5, M, MaxFileSizeLimit);
        {done, Req2} ->
            {ok, Req2}
    end.
